...
def RandK_compressor(x, k=200):
    """
    Top-k Compressor: Retain the k components with the largest |x|, set the rest to zero
    Args:
        x: Input vector (numpy 1-D array or array-like)
        k: Number of components to retain, default200
    Returns:
        vec: Sparse vector after processing
    """
    x = np.asarray(x)
    d = x.size
    if d == 0 or k <= 0:
    ...
    vec = np.zeros_like(x)
    vec[indices] = x[indices]
    return vec


def TopK_compressor(x, k=150):
    """
    Top-k Compressor: Retain the k components with the largest |x|, set the rest to zero
    Args:
        x: Input vector (numpy 1-D array or array-like)
        k: Number of components to retain, default 150
    Returns:
        vec: Sparse vector after processing
    """
    x = np.asarray(x)
    ...
    # Find the k indices with the largest absolute values
